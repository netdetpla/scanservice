...
def masscan():
    ...
    command = (
        '{masscan} -iL {target} -e '+output[:-1]+' -p{ports} -pU:{ports} -oJ {mid_json} --rate 1000 --exclude 10.0.0.0/8,192.168.0.0/16,172.16.0.0/12,127.0.0.1/8,0.0.0.0/8'
    ).format(
...
def shell_nmap():
    ...
    count = 0
    shell_command = './test.sh {extra}'.format(extra=extra_info)
    for host in host_list:
